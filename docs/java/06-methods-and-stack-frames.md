# Methods and Stack Frames

## Definition

A method is a reusable block of code. A stack frame is the runtime memory created for one method call.

## Why It Matters

This explains local variables, method calls, recursion, stack memory, and StackOverflowError.

## Core Example

When `main()` calls `add()`, the JVM creates a new stack frame for `add()` and removes it when the method returns.

## Common Traps

- Each method call gets a new stack frame.
- Local variables live in the stack frame.
- Objects created inside methods usually live in heap.
- Recursion can create many stack frames.
- Returning from a method removes its frame.

## Interview Answer

When a method is called, the JVM creates a stack frame for that call. The frame stores local variables, operand stack data, and return information. When the method finishes, its frame is removed. This is why deep recursion can cause StackOverflowError.

## Quick Revision

- Method call creates stack frame.
- Method return removes stack frame.
- Local variables are stored in frames.
- Objects usually live in heap.
