...
Overloading means same method name with different parameters. Overriding means a child class changes inherited method behavior.

## Why It Matters

This is a core polymorphism topic and a common source of compile-time vs runtime binding questions.

## Core Example

`print(int x)` and `print(String s)` are overloaded. `Dog.sound()` overriding `Animal.sound()` is overriding.
...
Method overloading happens in the same class or inheritance hierarchy when methods have the same name but different parameter lists. It is compile-time polymorphism. Method overriding happens when a subclass provides a new implementation of a parent instance method. It is runtime polymorphism.
...
- Return type alone is not enough for overload.
