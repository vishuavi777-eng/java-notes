...
Type conversion changes a value from one data type to another. Casting is explicit conversion written by the programmer.

## Why It Matters

Conversion rules explain compile errors, data loss, numeric promotion, and safe assignment in Java.

## Core Example

`int x = 10; long y = x;` is widening. `long a = 10; int b = (int) a;` is narrowing.
...
- Arithmetic with smaller integer types promotes to int.
- Casting does not make an invalid object type valid.

## Interview Answer

Java supports widening and narrowing conversion. Widening moves a smaller type to a larger compatible type and happens automatically. Narrowing moves a larger type to a smaller type and requires an explicit cast because data loss is possible.
...
- Object casts need compatible runtime type.
- Casting can fail with ClassCastException.
