...
This topic explains shared data, object-specific data, static methods, class loading, and common access mistakes.

## Core Example

`static int count` is shared by all objects, while `String name` is separate for each object.
...
- Static is not inheritance-based object polymorphism.
- Too much static state can make code hard to test.

## Interview Answer

Static members are associated with the class and are shared across all instances. Instance members are associated with a specific object. Static methods can be called using the class name, while instance methods need an object because they work with object state.
...
- Use class name for static access.
