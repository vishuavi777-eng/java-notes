...
This topic explains string pool behavior, performance, immutability, and thread-safety trade-offs.

## Core Example

Use StringBuilder when building a string inside a loop in single-threaded code.

## Common Traps

- String concatenation can create many objects.
- String is immutable.
- StringBuilder is not thread-safe.
- StringBuffer is synchronized but usually slower.
- `==` compares references, not string content.

## Interview Answer

String represents immutable text, so every modification creates a new string. StringBuilder is mutable and efficient for repeated modifications in single-threaded code. StringBuffer is also mutable, but its methods are synchronized, so it is safer for shared multi-threaded use but usually slower.
