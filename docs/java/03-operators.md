...
Operators appear in output questions, precedence traps, boolean logic, increments, comparisons, and bit-level interview questions.

## Core Example

`a + b` adds values, `a == b` compares primitive values or object references, and `a && b` performs short-circuit boolean logic.

## Common Traps

- `==` compares object references for objects.
- `&&` short-circuits but `&` evaluates both sides for booleans.
- Post-increment and pre-increment behave differently.
- Operator precedence can change output.
- Integer division drops decimal part.

## Interview Answer

Java operators perform arithmetic, comparison, logical, assignment, bitwise, and unary operations. In interviews, the important parts are precedence, short-circuit behavior, increment behavior, and the difference between reference comparison and logical equality.

## Quick Revision

- Arithmetic operators calculate values.
- Relational operators compare values.
- `==` checks references for objects.
- `&&` and `||` short-circuit.
- Pre/post increment changes timing.
