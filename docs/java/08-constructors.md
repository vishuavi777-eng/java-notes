...
A constructor is a special block used to initialize a new object.

## Why It Matters

Constructors explain object initialization, constructor chaining, default constructors, and common output traps.

## Core Example

`new Person("Asha")` allocates an object and then calls the matching constructor to initialize it.

## Common Traps

- Constructor name must match class name.
- Constructors do not have return types.
- Constructor does not allocate memory by itself.
- If no constructor is written, Java adds a default constructor.
- `this()` or `super()` must be first statement if used.

## Interview Answer

A constructor initializes an object after memory is allocated. It has the same name as the class and no return type. Java provides a default no-argument constructor only when no constructor is defined. Constructors can be overloaded and can call other constructors using `this()` or parent constructors using `super()`.
