...
An abstract class is a partial class that can hold state and behavior. An interface defines a contract that classes can implement.

## Why It Matters

This helps decide between inheritance-based reuse and capability-based design in Java applications.

## Core Example

`Vehicle` can be an abstract class with shared fields, while `Payable` can be an interface implemented by different unrelated classes.
...
- Use interface for capability or contract.
- Use abstract class for shared base state or behavior.

## Interview Answer

Use an interface when you want to define a contract or capability that many unrelated classes can implement. Use an abstract class when classes share common state or base behavior and have a natural parent-child relationship. Java allows multiple interfaces but only one direct class inheritance.

## Quick Revision

- Interface defines contract.
- Abstract class can share state.
- Class can implement many interfaces.
- Class can extend one class.
- Abstract class can have constructor.
- Prefer interface for flexible design.
