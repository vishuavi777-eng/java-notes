...
`final` restricts modification: final variables cannot be reassigned, final methods cannot be overridden, and final classes cannot be extended.

## Why It Matters

Final is common in immutability, constants, inheritance control, and interview traps around references.

## Core Example

`final List<String> names` means the reference cannot point to another list, but the list object may still be mutable.
...
- Compile-time constants have special behavior.

## Interview Answer

`final` means the target cannot be changed in a specific way. A final variable cannot be reassigned after initialization. A final method cannot be overridden by subclasses. A final class cannot be extended. For object references, final protects the reference, not necessarily the object contents.
...
- Final reference can point to mutable object.
- Final fields need initialization.
- Useful for constants and design safety.
