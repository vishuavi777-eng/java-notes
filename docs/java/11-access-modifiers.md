...
They are important for encapsulation, API design, package structure, and inheritance behavior.

## Core Example

`private` fields can be accessed only inside the same class. `public` methods can be accessed from anywhere.

## Common Traps

- Default access means package-private.
- Protected is not simply public to subclasses everywhere.
- Top-level classes can be public or package-private only.
- Private members are not inherited as accessible members.
- Access level cannot be reduced when overriding.

## Interview Answer

Java access modifiers define visibility. `private` is only inside the class. Default or package-private is inside the same package. `protected` is available in the same package and through inheritance rules. `public` is available everywhere.
...
- public: everywhere.
- Top-level class cannot be private/protected.
