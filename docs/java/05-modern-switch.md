...
Modern switch is the newer Java switch syntax that supports arrow labels and switch expressions.

## Why It Matters

It reduces boilerplate, avoids many fall-through bugs, and is common in newer Java codebases.

## Core Example

`String result = switch (day) { case MONDAY -> "Start"; default -> "Other"; };` returns a value from switch.

## Common Traps

- Old switch can fall through without break.
- Switch expressions must produce a value.
- `yield` can return a value from a block case.
- Arrow switch labels do not fall through.
- Exhaustiveness matters for switch expressions.

## Interview Answer

Modern switch improves old switch by using arrow syntax and allowing switch to be used as an expression. It reduces accidental fall-through and can return a value directly. For block cases, Java uses `yield` to provide the result.

## Quick Revision

- Arrow cases avoid fall-through.
- Switch can be an expression.
- Use yield in block cases.
- Old switch may need break.
- Modern switch is cleaner.
- Good for enum and fixed-value branching.
