...
A variable is a named storage location. A data type tells Java what kind of value the variable can hold.

## Why It Matters

This is the base for understanding memory, primitives, references, default values, constants, and type safety.

## Core Example

`int age = 25;` stores an integer value. `String name = "Java";` stores a reference to a String object.
...
- `final` variables are not always compile-time constants.
- Primitive variables and reference variables behave differently.

## Interview Answer

Variables store values used by a Java program. Primitive variables store simple values like `int`, `double`, and `boolean`. Reference variables store references to objects like `String` or custom classes. Java uses data types to provide compile-time safety and decide what operations are valid.
